...
const COLUMNS = [
  ...
  { key: "name", label: "Name" },
  { key: "date", label: "Date" },
  { key: "close_price_usd", label: "Close Price (USD)" },
];
...
export default function PricesPage() {
  ...
  const fetchData = async (params: Record<string, string>, pageNum: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/data/prices?${query.toString()}`);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold">Prices (OHLC Close)</h1>
      <DataFilters
