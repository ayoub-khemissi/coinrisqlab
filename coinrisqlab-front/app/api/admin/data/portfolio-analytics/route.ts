...
import { getPortfolioAnalytics, getPortfolioAnalyticsCount } from "@/lib/queries/data-validation";
...
export async function GET(request: NextRequest) {
  ...
  if (format === "csv") {
    const csv = toCsv(rows as Record<string, unknown>[], [
      "date", "window_days", "total_value_usd", "num_holdings", "data_points",
      "daily_volatility", "annualized_volatility", "diversification_benefit",
      "var_95", "var_99", "cvar_95", "cvar_99",
      "skewness", "kurtosis", "sharpe_ratio",
      "portfolio_beta_weighted", "beta_regression", "alpha_regression",
      "r_squared", "correlation_with_index",
    ]);

    return new NextResponse(csv, {
      headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": 'attachment; filename="portfolio_analytics_export.csv"' },
    });
  }
