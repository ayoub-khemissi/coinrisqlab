...
export default function MovingAveragePage() {
  ...
  const fetchData = async (params: Record<string, string>, pageNum: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/data/ma-90d?${query.toString()}`);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold">Moving Average</h1>
      <DataFilters
        csvEndpoint="/api/admin/data/ma-90d"
        csvFilename="moving_average_export.csv"
        defaultDays={90}
        loading={loading}
        metric="ma"
